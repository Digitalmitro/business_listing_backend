...
exports.createOffer = async (req, res) => {
  ...
  if (!businessId || !couponCode || !discountRate || !expirationDate || !categoryId) {
    return res.status(400).json({
      success: false,
      message: 'Missing required fields (businessId, couponCode, discountRate, expirationDate, categoryId)'
    });
  }
  ...
  try {
    ...
    if (!business.category.includes(categoryId)) {
      return res.status(400).json({ success: false, message: 'Invalid categoryId for this business' });
    }

    if (subCategoryId && !business.subCategory.includes(subCategoryId)) {
      return res.status(400).json({ success: false, message: 'Invalid subCategoryId for this business' });
    }
...
exports.getOffers = async (req, res) => {
  ...
  try {
    ...
    const offers = await Offer.find({ businessId: new mongoose.Types.ObjectId(businessId) })
