...
const claimWorker = new Worker(
  ...
  async (job) => {
    ...
    try {
      ...
      const subject = `Business Claim ${isApproved ? "Approved" : "Rejected"} - UrbanCitations`;
      
      const html = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;">
          <h2 style="color: ${isApproved ? "#4CAF50" : "#F44336"};">Claim ${isApproved ? "Approved" : "Rejected"}</h2>
          <p>Hi ${user.full_name},</p>
          <p>Your request to claim the business <strong>${claim.businessName}</strong> has been <strong>${status}</strong> by our team.</p>
