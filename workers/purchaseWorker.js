...
const purchaseWorker = new Worker(
  ...
  async (job) => {
    ...
    try {
      ...
      const subject = "Package Purchase Confirmation - UrbanCitations";
      const html = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;">
          <h2 style="color: #4CAF50;">Thank You for Your Purchase!</h2>
          <p>Hi ${user.full_name},</p>
          <p>Your purchase for the business <strong>${business.businessName}</strong> was successful.</p>
          <div style="background-color: #f9f9f9; padding: 15px; border-radius: 5px; margin: 20px 0;">
            <p style="margin: 5px 0;"><strong>Package:</strong> ${packageDetails.packageName}</p>
            <p style="margin: 5px 0;"><strong>Status:</strong> Active</p>
            <p style="margin: 5px 0;"><strong>Start Date:</strong> ${new Date().toLocaleDateString()}</p>
          </div>
          <p>Your business will now benefit from the premium features included in the ${packageDetails.packageName} plan.</p>
          <div style="text-align: center; margin: 30px 0;">
            <a href="${process.env.FRONTEND_URL}/dashboard" style="background-color: #4CAF50; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; font-weight: bold;">Go to Dashboard</a>
          </div>
